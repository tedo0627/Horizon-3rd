...
public class OverLifeGenerator extends Generator{
    ...
    public OverLifeGenerator() {
        this(new HashMap<>());
    }

    public OverLifeGenerator(Map<String, Object> options) {
    ...
    public void generateChunk(int chunkX, int chunkZ) {
        ...
        for (int genx = 0; genx < 16; genx++) {
            for (int genz = 0; genz < 16; genz++) {
        		...
        		int x = chunk.getX() * 16 + genx;
        		int z = chunk.getZ() * 16 + genz;
        		if (0 <= x && x <= 4) {
                    chunk.setBlock(genx, generateHeight, genz, 98);
        		}
        		if (0 <= z && z <= 4) {
                    chunk.setBlock(genx, generateHeight, genz, 98);
        		}
