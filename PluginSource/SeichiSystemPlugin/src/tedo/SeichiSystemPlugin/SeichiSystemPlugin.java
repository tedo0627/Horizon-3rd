...
import tedo.SeichiSystemPlugin.delayedTask.AsaultTask;
import tedo.SeichiSystemPlugin.delayedTask.AutoSaveTask;
import tedo.SeichiSystemPlugin.delayedTask.EffectTask;
import tedo.SeichiSystemPlugin.delayedTask.TopTask;
...
public class SeichiSystemPlugin extends PluginBase implements Listener{
	...
	public ArrayList<String> fly = new ArrayList<String>();
	public ArrayList<Player> cooltime = new ArrayList<Player>();
	...
	public HashMap<Player, Boolean> skill = new HashMap<Player, Boolean>();
	...
	public void onEnable(){
		...
		this.getServer().getPluginManager().registerEvents(new SeichiListener(this), this);
		...
		getServer().getScheduler().scheduleRepeatingTask(new AutoSaveTask(this), 20 * 60 * 10);
		getServer().getScheduler().scheduleRepeatingTask(new EffectTask(this), 20 * 60);
		getServer().getScheduler().scheduleRepeatingTask(new AsaultTask(this), 3);
		getServer().getScheduler().scheduleRepeatingTask(new TopTask(this), 20 * 60 * 30);
	}
	...
	public void onBlockUpdate(BlockUpdateEvent event) {
		...
		int id = block.getId();
		if (this.level.contains(block.getLevel().getName())) {
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		double y = player.y;
		double z = player.z;
		if (this.level.contains(level.getName())) {
...
//	@EventHandler
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (player.isSurvival()) {
			if (this.usedskill.get(name)) {
				if (this.cooltime.contains(player)) {
	...
	public void onEntitypawn(EntitySpawnEvent event) {
		...
		if (level.contains(entity.level.getName())) {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		Block block = event.getBlock();
		if (this.level.contains(block.getLevel().getName()) && block.y == 6 && block.getId() == 44) {
			event.setCancelled();
			return;
		}
			...
			if (sender instanceof Player) {
				try{
					if(args[0] != null){
						...
						switch (args[0]) {
							case "on":
								this.skill.put((Player) sender, true);
								sender.sendMessage("§a>>§bスキルをonにしました");
							break;

							case "off":
								this.skill.put((Player) sender, false);
	...
	public boolean skillBlockBreak(Player player, Vector3 pos, Level level) {
		...
		if (id != 0) {
			...
			if (id != 7) {
				if (this.level.contains(level.getName()) && block.y > 5 && (!(block.y == 6 && block.getId() == 44))) {
					...
					switch (b.getId()) {
						...
						case 0: case 7:
							...
							if (!(ev.isCancelled())) {
								...
								addCount(player, 1);
								sendStorage(player, ev.getDrops());
	...
	public void sendStorage(Player player, Item[] items) {
		for (int i = 0; i < items.length; i++) {
			Item it = items[i];
			int id = it.getId();
			if (id != 8 && id != 9 && id != 10 && id != 11) {
				this.item.addItem(player, items[i]);
			}
		}
	}
	...
	public void handBlockBreak(BlockBreakEvent event, boolean r) {
		...
		this.getServer().getPluginManager().callEvent(ev);
		this.usedskill.put(name, true);
		if (this.level.contains(level.getName()) && block.y < 6) {
			return;
		}else if (this.level.contains(level.getName()) && block.y == 6 && block.getId() == 44) {
			return;
		}
		if (!(ev.isCancelled())) {
			if (isAddDamage(item)) {
	...
	public void cooltask(Player player) {
		this.cooltime.remove(player);
	}
}
